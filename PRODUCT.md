...
- **Shifts** (`pages/shifts.html`) — work-hours + pay tracker. Log a shift (date, start/end, break, hourly rate, employer); it computes hours and pay (`ShiftEngine`, pure + tested, handles breaks and overnight shifts). Stats for this week / month / all-time (hours + earnings). "Log as income" creates a linked income transaction (`tags:['shift']`, kept in sync on edit/delete) so shift pay flows into balance, cash flow, insights and the forecast run-rate. Stored in the Supabase `shifts` table with a localStorage fallback until the schema is run (default hourly rate remembered locally). Linked from the sidebar + mobile More sheet.
