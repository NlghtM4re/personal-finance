...
- **Hours Tracker** (`pages/shifts.html`) — work-hours + pay tracker built for fast logging. Log a shift via day-of-week chips (Today / Yest / recent weekdays, with a date fallback), start/end, break, **tips**, and **either an hourly rate or a flat "Total paid"** amount. Pay/hours math is pure + tested (`ShiftEngine` — breaks, overnight shifts, fixed-or-hourly, tips). **Presets**: save the current shift as a named template and one-tap to pre-fill it next time (local). Stats for this week / month / all-time (hours + earnings); the list is grouped by week with subtotals. "Log as income" creates a linked income transaction (`tags:['shift']`, kept in sync on edit/delete) so shift pay flows into balance, cash flow, insights and the forecast run-rate. Stored in the Supabase `shifts` table (pay_mode/fixed_pay/tips columns) with a localStorage fallback until the schema is run; default rate + presets remembered locally. Linked from the sidebar + mobile More sheet.
