/* ============================================================
   icons.js — Lucide icons for transaction categories.
   Downloaded from lucide-static v0.544.0 (ISC license).
   Replaces emoji for the built-in categories; user-created
   custom categories keep their chosen emoji.
   ============================================================ */
...
const LUCIDE_ICONS = {
  ...
  /* extra icons for the custom-category picker */
...
/* Icons offered in the custom-category picker (order = display order). */
const PICKABLE_ICONS = [
  'tag', 'utensils', 'coffee', 'pizza', 'cake', 'shopping-cart', 'shopping-bag',
  'shirt', 'gift', 'house', 'bed', 'zap', 'droplet', 'flame', 'wifi', 'tv',
  'phone', 'smartphone', 'laptop', 'car', 'bus', 'bike', 'fuel', 'plane',
  'heart-pulse', 'heart', 'pill', 'dumbbell', 'scissors', 'baby', 'paw-print',
  'dog', 'gamepad-2', 'music', 'palette', 'camera', 'ticket', 'book-open',
  'star', 'sprout', 'wrench', 'wallet', 'credit-card', 'piggy-bank', 'banknote',
  'coins', 'landmark', 'receipt', 'briefcase', 'trending-up', 'shield',
  'hand-heart', 'package',
];

/* Build a clickable icon grid; writes the chosen name into `hiddenInput`. */
function renderIconPicker(gridEl, hiddenInput, selected) {
  if (!gridEl || !hiddenInput) return;
  gridEl.innerHTML = PICKABLE_ICONS.map(n =>
    `<button type="button" class="icon-pick-btn" data-icon="${n}" aria-label="${n}">${lucideSVG(n, 20)}</button>`
  ).join('');
  const mark = (name) => gridEl.querySelectorAll('.icon-pick-btn').forEach(b =>
    b.classList.toggle('selected', b.dataset.icon === name));
  gridEl.querySelectorAll('.icon-pick-btn').forEach(b => {
    b.addEventListener('click', () => { hiddenInput.value = b.dataset.icon; mark(b.dataset.icon); });
  });
  const initial = selected && LUCIDE_ICONS[selected] ? selected : 'tag';
  hiddenInput.value = initial;
  mark(initial);
}

/* built-in category id -> lucide icon name */
const CATEGORY_LUCIDE = {
  'cat-salary': 'briefcase', 'cat-freelance': 'laptop', 'cat-bonus': 'coins',
  'cat-refund': 'banknote', 'cat-gift': 'gift', 'cat-invest': 'trending-up',
  'cat-food': 'utensils', 'cat-groceries': 'shopping-cart', 'cat-rent': 'house',
  'cat-bills': 'zap', 'cat-subs': 'tv', 'cat-transport': 'car',
  'cat-health': 'heart-pulse', 'cat-fitness': 'dumbbell', 'cat-care': 'scissors',
  'cat-shopping': 'shopping-bag', 'cat-entertain': 'gamepad-2', 'cat-pets': 'paw-print',
  'cat-insurance': 'shield', 'cat-taxes': 'receipt', 'cat-donations': 'hand-heart',
  'cat-education': 'book-open', 'cat-travel': 'plane', 'cat-other': 'package',
};

function lucideSVG(name, size = 20) {
...
/* Category icon: a Lucide SVG for built-in categories, the chosen emoji
   for custom categories, else a package icon. `escapeHTML` from store.js. */
function categoryIconHTML(cat, size = 20) {
  if (cat) {
    const name = CATEGORY_LUCIDE[cat.id] || (LUCIDE_ICONS[cat.lucide] ? cat.lucide : null);
    if (name) return lucideSVG(name, size);
    if (cat.icon) {
      const esc = (typeof escapeHTML === 'function') ? escapeHTML(cat.icon) : cat.icon;
      return `<span class="cat-emoji">${esc}</span>`;
    }
  }
  return lucideSVG('package', size);
}
